...
while count < N or (hi-lo) < tol:
    ...
    Qc2 = Expression("Qfun*exp ( -pow( x[1] -( x1-x2 )/2, 2 )/( 2*pow( x1-x2,2 ) ) )", degree=1, Qfun=0.0, x1=0.3,
                     x2=0.1)
    ...
    for Gamma_val in [1, 5, 10, 15, 20, 23]:
        ...
        total_flux_int = assemble(flux)

       # print("Total flux on boundary 2 during continuation is", total_flux_int)

    # Extract flux
    flux = dot(u, n) * dot(u, n) * ds(1)
    total_flux_new = assemble(flux) - 1.5
    print("Total flux on boundary 2 is", total_flux_new)
    if total_flux_new < tol:
        hi = a
    else:
        lo = a
...
solve(F == 0, w, bcs)
# Plot solutions
File("Results/velocityCyl_uinm2p5.pvd") << u0
File("Results/pressureCyl_uinm2p5.pvd") << p0
File("Results/TemperatureCyl_uinm2p5.pvd") << T0
