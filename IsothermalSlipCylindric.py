...
a2 = (- div_cyl(u) * q1 ) * x[0] * dx
b1 = - dot(dot(sigma(u, p), v), n) * x[0] * ds(1)
b3 = - dot(dot(sigma(u, p), v), n) * x[0] * ds(3)
b4 = - dot(dot(sigma(u, p), v), n) * x[0] * ds(4)
F = a1 + a2
...
File("Results/StressIsothermalCyl" + mesh_type + "Slip.pvd") << sig

File("Results/velocityIsothermalCyl" + mesh_type + "Slip.pvd") << u
File("Results/pressureIsothermalCyl" + mesh_type + "Slip.pvd") << p
