...
ind = 0 # 0 is Cylindric, 1 is Cartesian. Let's not make it too complicated, definitely fix it later.
...
Gamma_values = [0.1, 0.5, 1, 5, 10, 15, 20, 21, 22, 23]
...
normal_stress1 = assemble(inner(sig*n, n)*ds(1))
values = np.asarray([normal_stress0, normal_stress1])
np.savetxt("Results/valuesCoupledSlip" + coord + val + "AdaptiveMesh.csv", values.T, delimiter='\t')
sav = 0.0
if sav == 1.0:
# Saving data
    np.savetxt("Results/valuesCoupledSlip" + coord + val + "AdaptiveMesh.csv", values.T, delimiter='\t')
    File("Results/velocityCoupledSlip" + coord + val +"AdaptiveMesh.pvd") << u
    File("Results/pressureCoupledSlip" + coord + val + "AdaptiveMesh.pvd") << p
    File("Results/TemperatureCoupledSlip" + coord + val + "AdaptiveMesh.pvd") << T
    File("Results/StressCoupledSlip" + coord + val + "AdaptiveMesh.pvd") << sig
    W2 = FunctionSpace(mesh, FiniteElement("CG", mesh.ufl_cell(), 1))
    Pmu = project(eta(), W2)

    File("Results/ViscosityCoupledSlip" + coord + val + "AdaptedMesh.pvd") << Pmu
