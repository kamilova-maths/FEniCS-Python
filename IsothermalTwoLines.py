...
for j in range(0, Nb):
    ...
    for i in range(0, Na):
        ...
        abnd = str(a)
        bbnd = str(b)
        CompiledSubDomain("near( ( ("+abnd+"-1) /(2*( 1 -" + bbnd + ")))*(x[0] - 1) +" + abnd + "- x[1], 0.0) && x[0]>=0.5").mark(colors, 2)
        CompiledSubDomain("near( ( (" + abnd + "-1) /(2*(" + bbnd + "-0.5)))*(x[0] - 0.5) + 1 - x[1], 0.0) && x[0]>=0.5").mark(colors, 2)
        ...
        flux_array_a.append(total_flux_new)
        ...
        a = a - da
    b = b + db
    flux_array_tot.append(flux_array_a)
    flux_array_a = []
...
fig = plt.figure()
#plt.contour([a_values, b_values], flux_array_tot)
#plt.plot(a_values, flux_array)
plt.ylabel('Flux')
plt.xlabel('a values')
plt.title('Sweep over a and b values, straight line two point free surface estimation')
plt.show()
