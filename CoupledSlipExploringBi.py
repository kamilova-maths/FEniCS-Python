...
normal_stress0_values = []
normal_stress1_values = []
Bi_range = [100, 80, 90, 70, 60, 50, 40, 30, 20, 10, 0]
...
if ind==1:
    ...
    for Bi_val in Bi_range:
        Bi.assign(Bi_val)
        print("Reduced Biot number is ", Bi_val)
        N = inner(F_v_cyl(u, grad_cyl(u)), grad_cyl(v)) * x[0] * dx - dot(f, v) * x[0] * dx + div_cyl(u) * q * x[
            0] * dx + dot(u, grad(T)) * S * x[0] * dx + \
        ...
        File("Results/velocityCoupledSlip" + coord + "Bi" + str(Bi_val) + "AdaptiveMesh.pvd") << u
        File("Results/pressureCoupledSlip" + coord +  "Bi" + str(Bi_val) + "AdaptiveMesh.pvd") << p
        File("Results/TemperatureCoupledSlip" + coord +  "Bi" + str(Bi_val) + "AdaptiveMesh.pvd") << T
        File("Results/StressCoupledSlip" + coord +  "Bi" + str(Bi_val) + "AdaptiveMesh.pvd") << sig
        W2 = FunctionSpace(mesh, FiniteElement("CG", mesh.ufl_cell(), 1))
        Pmu = project(eta(), W2)

        File("Results/ViscosityCoupledSlip" + coord +  "Bi" + str(Bi_val) + "AdaptedMesh.pvd") << Pmu
