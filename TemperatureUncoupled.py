...
F = (dot(u, grad(T))*v2 + (1/Pe)*inner(grad(v2), grad(T)) - v2*Qc2)*dx - (1/Pe)*(v2*Qc*ds(5) - Bi*v2*T*ds(3) + v2*Bi*Ta*ds(3)) # - dot(grad(T), v2*n)*ds(1)

# Solve problem
solve(F == 0, T, bcs)

# Plot solutions
File("Results/TemperatureUncoupledSym.pvd") << T
plot(T)
plt.show()
