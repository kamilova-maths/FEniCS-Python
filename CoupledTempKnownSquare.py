...
vsc = as_vector([v[0], v[1]])

# Cartesian
# a1 = inner(sigma(usc, p), epsilon(vsc)) * dx() - dot(f, vsc) * dx()
a = (inner(sigma(usc, p), epsilon(vsc)) - div(usc) * q) * dx()
b0 = - dot(dot(sigma2d(usc, p), vsc), n) * ds(0)
b2 = - dot(dot(sigma2d(usc, p), vsc), n) * ds(2)
b3 = -dot(dot(sigma2d(usc, p), vsc), n) * ds(3)
b4 = + dot(p*n, vsc) * ds(4)
...
vtkfile_u = File("Results/velocityTempEps" + Texp + ".pvd")

vtkfile_p = File("Results/pressureTempEps" + Texp + ".pvd")

vtkfile_mu = File("Results/ViscosityTempEps"+Texp+".pvd")
